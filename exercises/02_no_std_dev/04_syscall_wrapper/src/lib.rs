...
/// exclamation mark `!` indicates this function never returns (diverges)
/// loop是为了防止syscall3返回（虽然它不应该返回），以确保函数永远不会返回
/// 在实际的sys_exit实现中，syscall3应该导致进程终止，因此永远不会返回到调用者。但是为了满足Rust的类型系统，我们需要确保函数的返回类型是`!`（never type），表示它永远不会返回。通过在sys_exit函数中添加一个无限循环，我们可以确保即使syscall3意外返回，函数也不会继续执行下去，从而满足类型系统的要求。
pub fn sys_exit(code: i32) -> ! {
    ...
    loop {
        // In case syscall3 returns (which it shouldn't), loop indefinitely
    }
}
