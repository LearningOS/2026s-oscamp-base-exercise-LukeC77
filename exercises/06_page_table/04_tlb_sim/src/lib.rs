...
#[derive(Clone, Debug)]
pub struct TlbEntry {
    pub valid: bool, // 表示条目是否有效,false 表示无效（未使用或已被刷新）, 不能命中
    pub asid: u16,
    pub vpn: u64,
    pub ppn: u64,
    pub flags: u64,
...
impl Tlb {
    ...
    /// 
    /// 
    /// TLB 满后，insert 不再找“空位”（因为没有）
    /// 直接写 entries[fifo_ptr]
    /// 然后 fifo_ptr = (fifo_ptr + 1) % capacity
...
impl Mmu {
    ...
    /// 在页表中添加一条映射。 (asid, vpn) -> (ppn, flags) 的映射。
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        ...
        // TLB未命中，查找页表
        for (asid, mapping) in self.page_table.iter() {
